...
# demonstration in the vignettes/README. The panel990 package is the
# supported way to retrieve this data; this script keeps the shipped
# example reproducible without depending on panel990.
#
...
get_table <- function(table_name, year, select = NULL) {
  options(timeout = max(600, getOption("timeout")))
  url <- paste0(efile_root, table_name, "-", year, ".CSV")
...
p06 <- get_table(
  "F9-P06-T00-GOVERNANCE", year,
  select = c(id_cols, meta_cols, p06_vars)
)
p04 <- get_table(
  "F9-P04-T00-REQUIRED-SCHEDULES", year,
  select = c("OBJECTID", p04_vars)
)
p12 <- get_table(
  "F9-P12-T00-FINANCIAL-REPORTING", year,
  select = c("OBJECTID", p12_vars)
)
m01 <- get_table(
  "SM-P01-T00-NONCASH-CONTRIBUTIONS", year,
  select = c("OBJECTID", sm_vars)
)
