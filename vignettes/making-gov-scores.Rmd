...
The four download chunks above are shown for provenance and are **not run** when
this article is built — they pulled the 2018 training tables from a legacy NCCS
endpoint. For current retrieval (the efile v2.1 archive and the forthcoming
`panel990` package) see `vignette("download-data")`. To keep this article
reproducible offline, we load that saved copy of the raw 2018 data:

```{r load-train-raw}
# This article is built with the committed training data present (pkgdown, or a
# source checkout). When it is unavailable -- e.g. R CMD check rebuilds vignettes
# from the tarball, which excludes data-raw/ -- the remaining chunks are skipped
# so the article still builds without error.
candidates <- c("../data-raw/dat-train-raw.rda",
                "../../data-raw/dat-train-raw.rda",
                "data-raw/dat-train-raw.rda")
train_file <- candidates[file.exists(candidates)][1]
if (is.na(train_file)) {
  knitr::opts_chunk$set(eval = FALSE)
  message("Training data (data-raw/dat-train-raw.rda) not found; ",
          "remaining chunks are not evaluated.")
} else {
  load(train_file)   # dat_all_4, dat_all_6, dat_all_12, dat_all_M
}
```
...
+---------+----------------------------------------------+
| P 4_LI  | 1- (Answered no to P4_LINE_29 ANDP4_Line_30) |
| NE_2    | or (Answered yes to (P4_LINE_29              |
| 9_30    | OR P4_Line_30) AND answered yes to Schedule  |
|         | M question 31 )                                |
|         |                                              |
|         | 0 - Answered yes to (P4_LINE_29              |
|         | OR P4_Line_30) AND answered no to Schedule   |
|         | M Line 31                                   |
|         |                                              |
...
|     other officer, director, trustee,   | assets       |
|     or key employee?                    |              |
|                                         |              |
