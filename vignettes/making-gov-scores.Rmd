...
plot below that we have multicollinearity among some of our features. Let's explore some of these more to decide which ones we keep in our analysis.
...
Let's just keep P6_Line_15A as it describes the main point
that we care about in these line items. Additionally, most people aren't doing one or the other; most people do both or neither.
...
The original `fa` function will calculate factor scores, but since our factors are binary, the `fa$scores` output are not the factor scores we are interested in. Instead, we use the
...
-   $L$ = factor loading matrix $\in \mathbb{R}^{12 x 6}$ . This is
    calcultated from the `fa` function.
